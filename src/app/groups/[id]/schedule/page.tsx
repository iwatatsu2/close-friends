...
export default function SchedulePage() {
  ...
  return (
    <div className="max-w-lg mx-auto p-4">
      ...
      <h1 className="text-xl font-bold mb-4">遊べる日カレンダー</h1>
      ...
      <div className="space-y-2">
        {weekDates.map((date, i) => {
          ...
          return (
            <Card key={dateStr} className={`${isToday ? "ring-2 ring-primary" : ""} ${isPast ? "opacity-50" : ""}`}>
              <CardContent className="py-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-bold w-6 text-center ${i >= 5 ? "text-blue-500" : ""} ${i === 6 ? "text-red-500" : ""}`}>
                      {WEEKDAYS[i]}
                    </span>
                    <span className="text-sm">
                      {date.getMonth() + 1}/{date.getDate()}
                    </span>
                    {isToday && <span className="text-xs bg-primary text-primary-foreground px-1.5 py-0.5 rounded">今日</span>}
                  </div>
                  {!isPast && (
                    <Button
                      size="sm"
                      variant={myAvail ? "default" : "outline"}
      ...
      {editingDate && (
        <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50" onClick={() => setEditingDate(null)}>
          <div className="bg-background w-full max-w-lg rounded-t-xl p-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-semibold mb-3">遊べる時間帯</h3>
            ...
            <Button className="w-full" onClick={saveAvailability}>登録する</Button>
          </div>
        </div>
      )}
    </div>
